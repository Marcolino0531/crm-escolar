...
import {
  ...
  podeVerLote,
  tipoLote,
  type PermissoesLotes,
  type TipoLote,
} from "@/lib/rh-folhas";

interface FolhasSalvasProps {
...
const FolhasSalvas: React.FC<FolhasSalvasProps> = ({ schoolId, permissoes, refreshKey }) => {
  ...
  const carregarBatches = useCallback(async () => {
    ...
    const rows = (data ?? []) as unknown as (Omit<Batch, "tipo"> & { tipo: string | null })[];
    setBatches(
      rows
        .map((r) => ({ ...r, tipo: tipoLote(r.tipo) }))
        .filter((b) => podeVerLote(b.tipo, permissoes)),
    );
  }, [schoolId, permissoes]);
  ...
  const quitados = contagemQuitados(itens);
      ...
      <div className="p-4">
        {carregando ? (
          ...
          <p className="text-sm text-gray-400 italic">
            Nenhuma folha salva. Use "Salvar Folha de Pagamento" em Vale Transporte ou Salário.
          </p>
        ) : (
                    ...
                    >
                      ...
                      <span
                        className={`shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold ${
                          ehVt ? "bg-sky-100 text-sky-700" : "bg-violet-100 text-violet-700"
                        }`}
                      >
                        {ROTULO_TIPO_LOTE[b.tipo]}
                      </span>
                      <span className="text-sm font-medium text-gray-800 truncate">{b.title}</span>
                      {b.reference_month && (
                        <span className="text-xs text-gray-500 shrink-0">
                          · Ref. {b.reference_month}
                        </span>
                      )}
                      <span className="text-xs text-gray-500 shrink-0">
                      ...
                      {itensCarregando ? (
                        ...
                        <>
                          <p className="text-xs text-gray-500 mb-2">
                            {quitados.pagos} de {quitados.total} pago(s)
                            {quitados.quitado && (
