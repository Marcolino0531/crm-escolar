...
async function runCron(): Promise<Response> {
  ...
  for (const p of pendencias) {
    ...
    if (multipla) {
      // Valor total ATUALIZADO no dia do disparo = soma dos saldos de todos os
      // boletos em aberto (mês vigente + anteriores).
      const totalAtualizado = divida?.totalSaldo ?? p.valorTotalBoleto;
      const varsMultipla = {
        to: p.telefone,
