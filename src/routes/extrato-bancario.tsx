...
export const Route = createFileRoute("/extrato-bancario")({
  head: () => ({
    meta: [
      { title: "Extrato Bancário — School Hub" },
...
function Dashboard() {
  ...
  const { data: rules = [] } = useQuery({
    ...
    queryFn: async () => {
      return selectAll<Tables<"categorization_rules">>(() =>
  ...
  const tableRows = useMemo(() => {
    return rowsWithBalance.filter((t) => {
      ...
      if (kind === "e") {
        ...
        if (subcategoryFilter !== "all" && (t as any).sub_cost_center_id !== subcategoryFilter)
          return false;
        return true;
      }
      if (kind === "r") {
        ...
        if (items && items.length) {
          ...
          if (
            ...
            !items.some((it) => it.revenue_subcategory_id === subcategoryFilter)
          )
            return false;
          return true;
        }
        if ((t as any).revenue_category_id !== id) return false;
        if (subcategoryFilter !== "all" && (t as any).revenue_subcategory_id !== subcategoryFilter)
          return false;
        return true;
      }
      return true;
  ...
  const subcategoryOptions = useMemo(() => {
    ...
    if (kind === "e")
      opts = subCcs
        .filter((s: any) => s.cost_center_id === id)
        .map((s: any) => ({ id: s.id, name: s.name }));
    if (kind === "r")
      opts = rss
        .filter((s: any) => s.revenue_category_id === id)
        .map((s: any) => ({ id: s.id, name: s.name }));
  ...
  async function applyBulkCategories() {
    ...
    const patch =
      kind === "e"
        ? {
            cost_center_id: id,
            sub_cost_center_id: bulkSub || null,
            revenue_category_id: null,
            revenue_subcategory_id: null,
          }
        : {
            revenue_category_id: id,
            revenue_subcategory_id: bulkSub || null,
            cost_center_id: null,
  ...
  async function applyPendingRules() {
    ...
    for (const t of pendentes) {
      ...
      const patch =
        wantedKind === "revenue"
          ? {
              revenue_category_id: match.revenue_category_id,
              revenue_subcategory_id: match.revenue_subcategory_id ?? null,
              cost_center_id: null,
              sub_cost_center_id: null,
            }
          : {
              cost_center_id: match.cost_center_id,
              sub_cost_center_id: match.sub_cost_center_id ?? null,
              revenue_category_id: null,
              revenue_subcategory_id: null,
            };
  ...
  const uncategorized = filteredTxs
    .filter((t) => !t.cost_center_id && t.type === "saida")
    .reduce((s, t) => s + Number(t.amount), 0);
  if (uncategorized > 0)
    byCC.push({ id: null, name: "Sem categoria", total: uncategorized, color: "#94a3b8" });
  ...
  if (revBySubId.uncategorized > 0)
    byRev.push({
      id: null,
      name: "Sem subcategoria",
      total: revBySubId.uncategorized,
      color: "#94a3b8",
    });
  ...
  const drillData = useMemo(() => {
    ...
    if (isExpense) {
      const matching = filteredTxs.filter(
        (t) => t.type === "saida" && (t.cost_center_id ?? null) === drill.id,
      );
        ...
        for (const it of items) {
          ...
          if (it.revenue_subcategory_id) {
            subTotals.set(
              ...
              (subTotals.get(it.revenue_subcategory_id) ?? 0) + Number(it.amount),
            );
          } else {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard
          label="Saldo Inicial"
          value={formatBRL(startingBalance)}
          icon={Wallet}
          tone={startingBalance >= 0 ? "success" : "destructive"}
        />
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={byCC} margin={{ top: 10, right: 16, left: 0, bottom: 8 }}>
                  ...
                  <Bar
                    ...
                    cursor="pointer"
                    onClick={(d: any) =>
                      setDrill({ kind: "expense", id: d.id, name: d.name, color: d.color })
                    }
                  >
                    {byCC.map((d, i) => (
          ...
          <div className="flex flex-wrap items-end gap-2">
            ...
            {isAdmin && (
              ...
              >
                ...
                {applyingRules ? "Aplicando…" : "Aplicar Regras Pendentes"}
              </Button>
            )}
            <Button
              variant="outline"
                  ...
                  {tableRows.map((t) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td
                          className={`py-2 pr-4 text-right font-medium ${isEntrada ? "text-success" : "text-destructive"}`}
                        >
                          {isEntrada ? "+" : "-"}
                          {formatBRL(Number(t.amount))}
                        </td>
                        <td
                          className={`py-2 pr-4 text-right tabular-nums ${t.running >= 0 ? "" : "text-destructive"}`}
                        >
                          {formatBRL(t.running)}
                        </td>
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              {drill && (
                <span className="h-3 w-3 rounded-full" style={{ background: drill.color }} />
              )}
              Detalhamento: {drill?.name}
              {drillData && (
                <span className="ml-2 text-muted-foreground font-normal">
                  — {formatBRL(drillData.total)}
                </span>
              )}
            </DialogTitle>
          </DialogHeader>
          {drillData &&
            (drillData.subs.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Nenhuma subcategoria com lançamentos no período.
              </p>
            ) : (
...
function EditTransactionDialog({
  ...
  const filteredSubCcs = useMemo(
    () => subCcs.filter((s) => s.cost_center_id === ccId),
    [subCcs, ccId],
  );
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-[11px] text-muted-foreground">
                {isNaN(parsedAmount)
                  ? "Valor inválido"
                  : `${isEntrada ? "Entrada" : "Saída"}: ${formatBRL(Math.abs(parsedAmount))}`}
              </p>
            </div>
          </div>
          <div>
          ...
          {isEntrada ? (
            <div className="grid grid-cols-2 gap-3">
                      ...
                      .map((rc: any) => (
                        <SelectItem key={rc.id} value={rc.id}>
                          <span className="inline-flex items-center gap-2">
                            <span
                              className="h-2 w-2 rounded-full"
                              style={{ background: rc.color ?? "#10b981" }}
                            />
                            {rc.name}
                          </span>
                        </SelectItem>
              ...
              <div>
                ...
                <Select
                  value={rsId}
                  onValueChange={setRsId}
                  disabled={!rcId || filteredRss.length === 0}
                >
                  <SelectTrigger>
                    <SelectValue
                      placeholder={
...
function InitialBalanceDialog({
  ...
  async function save() {
    ...
    const { error } = await supabase.from("initial_balances" as any).upsert(
      ...
      { onConflict: "school_id,reference_date" },
    );
    if (error) return toast.error(error.message);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          ...
          size="sm"
          disabled={disabled}
          title={disabled ? "Selecione um colégio" : ""}
        >
        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              O Dashboard usa o saldo inicial mais recente até a data inicial do filtro.
            </p>
          </div>
          <div>
  ...
  return (
    <Card>
      <CardContent className="flex items-center gap-4 p-5">
        <div
          className={`flex h-11 w-11 items-center justify-center rounded-lg ${tone === "success" ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}
        >
