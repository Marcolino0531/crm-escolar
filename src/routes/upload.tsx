...
function UploadPage() {
  ...
  function guess(desc: string, type: "entrada" | "saida", amount?: number): {
    costCenterId: string | null;
    subCostCenterId: string | null;
    revenueCategoryId: string | null;
    revenueSubcategoryId: string | null;
    ...
    return { costCenterId: null, subCostCenterId: null, revenueCategoryId: null, revenueSubcategoryId: null, matched: false };
  }
  ...
  async function handleFile(file: File) {
    ...
    toast.success(`${parsed.length} transações lidas. ${matched} categorizadas. ${dups > 0 ? `${dups} possível(eis) duplicata(s).` : ""}`);
  }
  ...
  function addManualRow() {
    ...
    setRows(prev => [...prev, {
      id, date: today, description: "", amount: 0, type: "saida",
      costCenterId: null, subCostCenterId: null,
      revenueCategoryId: null, revenueSubcategoryId: null,
      guessed: false, duplicate: false, dismissedDup: false, manual: true, notes: "",
    }]);
  }
          ...
          <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-3 space-y-0">
            <div>
              <CardTitle className="flex items-center gap-2">
                ...
                Pré-visualização ({rows.length}) {schoolId && `— ${schools.find(s => s.id === schoolId)?.name}`}
              </CardTitle>
            ...
            <div className="flex flex-wrap gap-2">
              <CategoryManagerDialog
                trigger={<Button variant="outline" size="sm"><Pencil className="h-4 w-4" /> Categorias</Button>}
              />
              {rows.length > 0 && (
                <>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {rows.map(r => {
                    ...
                    const rowBg = isDup ? "bg-warning/10" : ok ? "bg-success/5" : "bg-destructive/5";
                    return (
                      <tr key={r.id} className={`border-b border-border/50 transition-colors ${rowBg}`}>
                        <td className="py-2 pr-3">
                          {isDup ? (
                            <div className="flex flex-col gap-1">
                              ...
                              <button onClick={() => patch(r.id, { dismissedDup: true })} className="text-[10px] underline text-muted-foreground hover:text-foreground">
                                Estou ciente, manter
                              </button>
                            </div>
                          ) : r.manual ? (
                            <span className="inline-flex items-center gap-1 rounded-full bg-accent/30 px-2 py-0.5 text-xs font-medium">Manual</span>
                          ) : auto ? (
                        ...
                        <td className="py-2 pr-3 whitespace-nowrap">
                          {r.manual ? (
                            <Input type="date" value={r.date} onChange={e => patch(r.id, { date: e.target.value })} className="h-8 w-36" />
                          ) : (
                            formatDateBR(r.date)
                          )}
                        </td>
                        <td className="py-2 pr-3">
                          {r.manual ? (
                            <Input value={r.description} onChange={e => patch(r.id, { description: e.target.value })} placeholder="Descrição…" className="h-8 min-w-[200px]" />
                          ) : (
                            r.description
                          )}
                        </td>
                        <td className={`py-2 pr-3 text-right font-medium ${isEntry ? "text-success" : "text-foreground"}`}>
                          {r.manual ? (
                            <div className="flex items-center gap-1 justify-end">
                              <Select value={r.type} onValueChange={(v: "entrada" | "saida") => patch(r.id, { type: v, costCenterId: null, subCostCenterId: null, revenueCategoryId: null, revenueSubcategoryId: null })}>
                              ...
                              <Input type="number" step="0.01" min="0" value={r.amount || ""} onChange={e => patch(r.id, { amount: parseFloat(e.target.value) || 0 })} className="h-8 w-24 text-right" />
                            </div>
                          ) : (
                        ...
                        <td className="py-2 pr-3 min-w-[200px]">
                          {isEntry ? (
                            <Select value={r.revenueCategoryId ?? undefined} onValueChange={(v) => patch(r.id, { revenueCategoryId: v, revenueSubcategoryId: null })}>
                              <SelectTrigger className="h-8">
                                <SelectValue placeholder={revCats.length === 0 ? "Cadastre categorias…" : "Categoria de receita…"} />
                              </SelectTrigger>
                              <SelectContent>
                                {[...revCats].sort((a: any, b: any) => a.name.localeCompare(b.name)).map(c => (
                                  <SelectItem key={c.id} value={c.id}>
                                    <span className="inline-flex items-center gap-2">
                                      <span className="h-2 w-2 rounded-full" style={{ background: (c as any).color ?? "#10b981" }} />
                                      {c.name}
                                    </span>
                                  </SelectItem>
                                ))}
                            ...
                            <Select value={r.costCenterId ?? undefined} onValueChange={(v) => patch(r.id, { costCenterId: v, subCostCenterId: null })}>
                              <SelectTrigger className="h-8">
                                <SelectValue placeholder="Selecionar…" />
                              </SelectTrigger>
                              <SelectContent>
                                {[...ccs].sort((a: any, b: any) => a.name.localeCompare(b.name)).map(cc => (
                        ...
                        <td className="py-2 pr-3 min-w-[180px]">
                          {isEntry ? (() => {
                            const filtered = revSubs.filter(s => s.revenue_category_id === r.revenueCategoryId).sort((a: any, b: any) => a.name.localeCompare(b.name));
                            return (
                              <Select
                                value={r.revenueSubcategoryId ?? undefined}
                                onValueChange={(v) => patch(r.id, { revenueSubcategoryId: v || null })}
                                disabled={!r.revenueCategoryId || filtered.length === 0}
                              >
                                <SelectTrigger className="h-8">
                                  <SelectValue placeholder={!r.revenueCategoryId ? "—" : filtered.length === 0 ? "Sem subcategorias" : "Opcional…"} />
                                </SelectTrigger>
                                <SelectContent>
                                  {filtered.map(s => (
                                    <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>
                            ...
                            const filtered = subs.filter(s => s.cost_center_id === r.costCenterId).sort((a: any, b: any) => a.name.localeCompare(b.name));
                            return (
                              <Select
                                value={r.subCostCenterId ?? undefined}
                                onValueChange={(v) => patch(r.id, { subCostCenterId: v || null })}
                                disabled={!r.costCenterId || filtered.length === 0}
                              >
                                <SelectTrigger className="h-8">
                                  <SelectValue placeholder={!r.costCenterId ? "—" : filtered.length === 0 ? "Sem subcentros" : "Opcional…"} />
                                </SelectTrigger>
                                <SelectContent>
                                  {filtered.map(s => (
                                    <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>
                        ...
                        <td className="py-2 pr-1 text-right">
                          <Button size="sm" variant="ghost" onClick={() => removeRow(r.id)} title="Excluir linha">
